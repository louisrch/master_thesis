...
from networks import Double_Q_Net, QNetwork, Policy_Net, GaussianPolicy, DeterministicPolicy, ReplayBuffer
...
class SAC(object):
    def __init__(self, **kwargs):
        ...
        self.critic = QNetwork(self.state_dim, self.action_dim, self.hid_shape[0]).to(device=self.dvc)
        self.critic_optim = torch.optim.Adam(self.critic.parameters(), lr=self.lr)

        self.critic_target = QNetwork(self.state_dim, self.action_dim, self.hid_shape[0]).to(self.dvc)
    ...
    def update_parameters(self, memory : ReplayBuffer, batch_size : int, updates : int):
        ...
        with torch.no_grad():
            ...
            print(next_state_batch, next_state_action)
